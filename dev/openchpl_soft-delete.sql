...
BEGIN
	IF NEW.deleted = true THEN
		...
		UPDATE openchpl.complaint_to_complaint_type_map as src SET deleted = NEW.deleted WHERE src.complaint_id = NEW.complaint_id;
    END IF;
	RETURN NEW;
END;
$$ language 'plpgsql';
...
BEGIN
	IF NEW.deleted = true THEN
		...
		UPDATE openchpl.quarterly_report_surveillance_cap_status_map as src SET deleted = NEW.deleted WHERE src.quarterly_report_surveillance_map_id = NEW.id;
    END IF;
	RETURN NEW;
END;
$$ language 'plpgsql';
