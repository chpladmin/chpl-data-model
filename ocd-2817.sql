     ...
     LEFT JOIN ( SELECT vshistory.vendor_status_id,
           ...
           FROM openchpl.vendor_status_history vshistory
             JOIN ( SELECT vendor_status_history.vendor_id,
                  ...
                  GROUP BY vendor_status_history.vendor_id) vsinner ON vshistory.vendor_id = vsinner.vendor_id AND vshistory.status_date = vsinner.status_date AND vshistory.deleted = false) vendorstatus ON vendorstatus.vendor_id = h.vendor_id
...
CREATE OR REPLACE VIEW openchpl.certified_product_search AS
 SELECT cp.certified_product_id,
   ...
   FROM openchpl.certified_product cp
     LEFT JOIN ( SELECT cse.certification_status_id,
           ...
           FROM openchpl.certification_status_event cse
             JOIN ( SELECT certification_status_event.certified_product_id,
                  ...
                  GROUP BY certification_status_event.certified_product_id) cseinner ON cse.certified_product_id = cseinner.certified_product_id AND cse.event_date = cseinner.event_date AND cse.deleted = false) certstatusevents ON certstatusevents.certified_product_id = cp.certified_product_id
     LEFT JOIN ( SELECT certification_status.certification_status_id,
            certification_status.certification_status AS certification_status_name
     ...
     LEFT JOIN ( SELECT muu.meaningful_use_users,
           ...
           FROM openchpl.meaningful_use_user muu
             JOIN ( SELECT meaningful_use_user.certified_product_id,
                  ...
                  WHERE meaningful_use_user.deleted <> true
                  GROUP BY meaningful_use_user.certified_product_id
				  FETCH FIRST 1 ROWS ONLY) muuinner ON muu.certified_product_id = muuinner.certified_product_id AND muu.meaningful_use_users_date = muuinner.meaningful_use_users_date
          WHERE muu.deleted <> true) muuresult ON muuresult.certified_product_id = cp.certified_product_id
     LEFT JOIN ( SELECT string_agg(DISTINCT (children.child_chpl_product_number::text || '☹'::text) || children.child_listing_id::text, '☹'::text) AS child,
            children.parent_listing_id
           FROM ( SELECT certified_product.certified_product_id,
                    listing_to_listing_map.parent_listing_id,
                    listing_to_listing_map.child_listing_id,
                    ( SELECT get_chpl_product_number.chpl_product_number
                           FROM openchpl.get_chpl_product_number(listing_to_listing_map.child_listing_id) get_chpl_product_number(chpl_product_number)) AS child_chpl_product_number
          ...
          GROUP BY children.parent_listing_id) child ON cp.certified_product_id = child.parent_listing_id
     LEFT JOIN ( SELECT string_agg(DISTINCT (parents.parent_chpl_product_number::text || '☹'::text) || parents.parent_listing_id::text, '☹'::text) AS parent,
            parents.child_listing_id
           FROM ( SELECT certified_product.certified_product_id,
                    listing_to_listing_map.child_listing_id,
                    listing_to_listing_map.parent_listing_id,
                    ( SELECT get_chpl_product_number.chpl_product_number
                           FROM openchpl.get_chpl_product_number(listing_to_listing_map.parent_listing_id) get_chpl_product_number(chpl_product_number)) AS parent_chpl_product_number,
                    certified_product.chpl_product_number
                   FROM openchpl.listing_to_listing_map
          ...
          GROUP BY parents.child_listing_id) parent ON cp.certified_product_id = parent.child_listing_id
     LEFT JOIN ( SELECT certification_edition.certification_edition_id,
            certification_edition.year
     ...
     LEFT JOIN ( SELECT vendor_1.vendor_id,
     ...
     LEFT JOIN ( SELECT vshistory.vendor_status_id,
            ...
            vshistory.status_date AS last_vendor_status_change
           FROM openchpl.vendor_status_history vshistory
             JOIN ( SELECT vendor_status_history_1.vendor_id,
                    max(vendor_status_history_1.status_date) AS status_date
                   FROM openchpl.vendor_status_history vendor_status_history_1
                  WHERE vendor_status_history_1.deleted = false
                  GROUP BY vendor_status_history_1.vendor_id) vsinner ON vshistory.vendor_id = vsinner.vendor_id AND vshistory.status_date = vsinner.status_date) vendor_status_history ON vendor_status_history.vendor_id = vendor.vendor_id
     LEFT JOIN ( SELECT vendor_status_1.vendor_status_id,
            vendor_status_1.name AS vendor_status_name
           FROM openchpl.vendor_status vendor_status_1) vendor_status ON vendor_status_history.vendor_status_id = vendor_status.vendor_status_id
     LEFT JOIN ( SELECT string_agg(vendor_1.name::text, '|'::text) AS history_vendor_name,
          ...
          GROUP BY product_owner_history_map.product_id) owners ON owners.history_product_id = product.product_id
     LEFT JOIN ( SELECT min(certification_status_event.event_date) AS certification_date,
            certification_status_event.certified_product_id
           FROM openchpl.certification_status_event
          WHERE certification_status_event.certification_status_id = 1 AND certification_status_event.deleted = false
     ...
     LEFT JOIN ( SELECT max(certification_status_event.event_date) AS decertification_date,
            certification_status_event.certified_product_id
           FROM openchpl.certification_status_event
          WHERE (certification_status_event.certification_status_id = ANY (ARRAY[3::bigint, 4::bigint, 8::bigint])) AND certification_status_event.deleted = false
          GROUP BY certification_status_event.certified_product_id) decert ON cp.certified_product_id = decert.certified_product_id
     LEFT JOIN ( SELECT surveillance.certified_product_id,
            count(*) AS count_surveillance_activities
           FROM openchpl.surveillance
     ...
     LEFT JOIN ( SELECT surv.certified_product_id,
            count(*) AS count_open_nonconformities
           FROM openchpl.surveillance surv
             ...
             JOIN openchpl.surveillance_nonconformity surv_nc ON surv_req.id = surv_nc.surveillance_requirement_id AND surv_nc.deleted <> true
             JOIN openchpl.nonconformity_status nc_status ON surv_nc.nonconformity_status_id = nc_status.id
          WHERE surv.deleted <> true AND nc_status.name::text = 'Open'::text
     ...
     LEFT JOIN ( SELECT surv.certified_product_id,
           ...
           FROM openchpl.surveillance surv
             ...
             JOIN openchpl.surveillance_nonconformity surv_nc ON surv_req.id = surv_nc.surveillance_requirement_id AND surv_nc.deleted <> true
             JOIN openchpl.nonconformity_status nc_status ON surv_nc.nonconformity_status_id = nc_status.id
          WHERE surv.deleted <> true AND nc_status.name::text = 'Closed'::text
          GROUP BY surv.certified_product_id) nc_closed ON cp.certified_product_id = nc_closed.certified_product_id
     LEFT JOIN ( SELECT certification_result.certified_product_id,
            string_agg(DISTINCT certification_criterion.number::text, '☺'::text) AS cert_number
          ...
          WHERE certification_result.success = true AND certification_result.deleted = false AND certification_criterion.deleted = false
          GROUP BY certification_result.certified_product_id) certs ON certs.certified_product_id = cp.certified_product_id
     LEFT JOIN ( SELECT string_agg(DISTINCT (certification_criterion.number::text || '☹'::text) || certification_result.api_documentation::text, '☺'::text) AS cert_number,
            certification_result.certified_product_id
           FROM openchpl.certification_criterion
             JOIN openchpl.certification_result ON certification_criterion.certification_criterion_id = certification_result.certification_criterion_id
          WHERE certification_result.success = true AND certification_result.api_documentation IS NOT NULL AND certification_result.deleted = false AND certification_criterion.deleted = false
          GROUP BY certification_result.certified_product_id) certs_with_api_documentation ON certs_with_api_documentation.certified_product_id = cp.certified_product_id
     LEFT JOIN ( SELECT string_agg(DISTINCT COALESCE(cqm_criterion.cms_id, ('NQF-'::text || cqm_criterion.nqf_number::text)::character varying)::text, '☺'::text) AS cqm_number,
          ...
          WHERE cqm_result.success = true AND cqm_result.deleted = false AND cqm_criterion.deleted = false
          GROUP BY cqm_result.certified_product_id) cqms ON cqms.certified_product_id = cp.certified_product_id
