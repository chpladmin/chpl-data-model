...
-- Modify the certified_product_details view for performance
...
CREATE OR REPLACE VIEW openchpl.certified_product_details AS
 SELECT a.certified_product_id,
    ...
    COALESCE(a.chpl_product_number, (((((((((((((((("substring"(b.year::text, 3, 2) || '.'::text) || q.testing_lab_code::text) || '.'::text) || c.certification_body_code::text) || '.'::text) || h.vendor_code::text) || '.'::text) || a.product_code::text) || '.'::text) || a.version_code::text) || '.'::text) || a.ics_code::text) || '.'::text) || a.additional_software_code::text) || '.'::text) || a.certified_date_code::text)::character varying) AS chpl_product_number,
