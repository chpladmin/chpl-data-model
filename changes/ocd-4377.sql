...
AS $function$
BEGIN
	IF NEW.last_modified_user IS NULL && NEW.last_modified_sso_user IS NULL THEN
		RAISE EXCEPTION 'Column last_modified_user or last_modified_sso_user requires a value.';
	ELSIF  NEW.last_modified_user IS NOT NULL && NEW lsat_modified_sso_user IS NOT NULL THEN
...
BEGIN
	...
	cmd := format('CREATE CONSTRAINT TRIGGER %I_last_modified_user_constraint AFTER INSERT OR UPDATE ON %I.%I DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE PROCEDURE openchpl.last_modified_user_constraint();', row.tablename, row.schemaname, row.tablename);
	RAISE NOTICE '%', cmd;
    END LOOP;
END
$$ LANGUAGE plpgsql;
