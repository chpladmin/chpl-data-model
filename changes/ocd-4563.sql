  ...
  RETURNS void AS $$
	...
	BEGIN
		FOR unique_subscription_rec IN
			SELECT DISTINCT subscriber_id, subscribed_object_id FROM openchpl.subscription
		LOOP
			...
			IF (SELECT COUNT(*) FROM openchpl.subscription WHERE subscriber_id = unique_subscription_rec.subscriber_id AND subscription_subject_id = subject_id AND subscribed_object_id = unique_subscription_rec.subscribed_object_id AND subscription_consolidation_method_id = 1 AND deleted = false)>0 THEN
				...
				RAISE NOTICE 'Subscription for subscriber %s, subject %s, object %s already exists', unique_subscription_rec.subscriber_id, subject_id, unique_subscription_rec.subscribed_object_id;
			ELSE
				-- insert the subscription
				INSERT INTO openchpl.subscription (subscriber_id, subscription_subject_id, subscribed_object_id, subscription_consolidation_method_id, last_modified_user)
				VALUES (unique_subscription_rec.subscriber_id, subject_id, unique_subscription_rec.subscribed_object_id, 1, -1);
				RAISE NOTICE 'Added subscription for subscriber %s, subject %s, object %s already exists', unique_subscription_rec.subscriber_id, subject_id, unique_subscription_rec.subscribed_object_id;
			END IF;
