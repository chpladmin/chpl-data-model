...
select 'DEV', 
        ...
        -1
where not exists (
        select * from openchpl.report_metadata where environment = 'DEV' and report_key = 'SVAPUsage'
);
...
select 'QA', 
        ...
        -1
where not exists (
        select * from openchpl.report_metadata where environment = 'QA' and report_key = 'SVAPUsage'
);
...
select 'STG', 
        ...
        -1
where not exists (
        select * from openchpl.report_metadata where environment = 'STG' and report_key = 'SVAPUsage' 
);
...
select 'PROD', 
        ...
        -1
where not exists (
        select * from openchpl.report_metadata where environment = 'PROD' and report_key = 'SVAPUsage' 
);
